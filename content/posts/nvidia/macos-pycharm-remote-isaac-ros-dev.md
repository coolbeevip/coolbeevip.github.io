...
- Git
- SSH key
- 可选：JetBrains Gateway，或者直接用 PyCharm 内置 Remote Development
...
### 3.2 使用 Remote Development 连接远程主机

这是更推荐的连接方式。PyCharm 的后端运行在远程主机，macOS 上只是显示 UI。对大型 ROS 2 workspace 更友好，因为索引、搜索、解释器探测都发生在远程主机上。
...
打开后，PyCharm 的 Terminal 默认在远程主机上，而不是 macOS 本地。可以直接执行：
...
### 3.3 可选：SSH Interpreter + Deployment

如果不想使用完整 Remote Development，也可以使用传统 SSH Interpreter。这个模式适合较小的 Python 节点，但对大型 ROS 2 workspace 的索引体验通常不如 Remote Development。
...
3. 添加解释器，选择 `On SSH`。
4. 填写远程主机信息。
5. 按提示选择远程 Python 解释器。
6. 在 `Tools | Deployment` 中配置自动同步。

注意：SSH Interpreter 选到的是远程主机上的 Python，不一定是 Isaac ROS 容器里的 Python。如果你的节点依赖容器内的 ROS 2 Python 包，运行和调试仍建议从 `isaac-ros activate` 后的 Terminal 发起。
...
| 方案                              | 适用场景                          | 取舍                                |
|---------------------------------|-------------------------------|-----------------------------------|
| Terminal + `isaac-ros activate` | `ros2 run`、`ros2 launch`、`colcon build` | **推荐**。使用官方 Docker 隔离环境，和实际运行环境一致 |
| Docker Interpreter              | 想让 PyCharm 直接在容器环境里运行 Python  | 也推荐，但 Isaac ROS CLI 管理的容器需要额外适配   |
| 远程主机 Python                     | 只做普通 Python 编辑和轻量脚本           | 仅适合作为索引和补全兜底，不推荐用于运行 Isaac ROS 节点 |

推荐做法是：

- 环境隔离采用 Docker，不把 Isaac ROS 依赖直接铺到远程主机系统 Python 里。
- PyCharm 解释器可以先配置为远程主机 Python，用于代码索引和基础补全。
- 真正运行 ROS 2 节点时，在 PyCharm Terminal 中进入 `isaac-ros activate`，也就是进入 Isaac ROS Docker 开发环境。
- 需要断点调试 Python 节点时，再按项目情况配置 PyCharm remote debug。

## 5. 让 PyCharm 更好识别 ROS 2 包

PyCharm 对 ROS 2 workspace 的理解不如专门的 ROS 工具，因此需要做几件事：
...
不要把 `build`、`install`、`log` 标记为源码目录。它们应该排除或保持普通目录，否则索引会变慢。
...
如果已经通过 `isaac-ros activate` 进入容器，通常容器基础环境已经处理了一部分，但 workspace 自己的 `install/setup.bash` 仍应在构建后 source。
...
Isaac ROS workspace 经常包含 rosbag、模型、TensorRT engine 和构建缓存，不排除会明显拖慢索引。
...
### 8.2 `add-apt-repository universe` 触发 Docker 源握手失败
...
### 8.3 PyCharm 可以索引，但提示 import 不存在
