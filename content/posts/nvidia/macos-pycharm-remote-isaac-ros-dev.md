...
这是更推荐的方式。PyCharm 的后端运行在远程主机，macOS 上只是显示 UI。对大型 ROS 2 workspace 更友好，因为索引、搜索、解释器探测都发生在远程主机上。
...
### 4.2 SSH Interpreter + Deployment
...
| 方案                              | 适用场景                          | 取舍                                |
|---------------------------------|-------------------------------|-----------------------------------|
| Terminal + `isaac-ros activate` | Isaac ROS 节点、launch、colcon 构建 | **推荐**。使用官方 Docker 隔离环境，和实际运行环境一致 |
...
### 9.2 `add-apt-repository universe` 触发 Docker 源握手失败
...
### 9.3 PyCharm 可以索引，但提示 import 不存在
