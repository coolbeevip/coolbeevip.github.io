...
远程主机使用 x86_64 Ubuntu 24.04 + NVIDIA GPU，ROS 发行版使用 Jazzy。先从 macOS 登录远程主机：
...
确认结果：macOS 能稳定 SSH 登录远程主机，`nvidia-smi` 能看到 NVIDIA GPU 和驱动信息，`lsb_release -a` 显示 Ubuntu 24.04。
...
5. 在远程主机上执行 `echo $ISAAC_ROS_WS`，拿到 workspace 的实际路径。
6. 在 PyCharm 中选择该实际路径，例如 `/home/user/workspaces/isaac_ros-dev`。
7. 等待 PyCharm 在远程主机安装 IDE backend。
...
3. 端口填写 `5678`。
4. 配置 path mappings：容器内代码路径映射到 PyCharm 打开的 workspace 路径。例如容器内 `/workspaces/isaac_ros-dev` 映射到远程主机 `${ISAAC_ROS_WS}` 对应的实际路径。
5. 启动这个 debug server。
...
安装调试包。优先使用 PyCharm `Python Debug Server` 配置页生成的精确安装命令，保证 `pydevd-pycharm` 版本与当前 PyCharm 匹配。形式通常如下：

```bash
python3 -m pip install "pydevd-pycharm~=PYCHARM_VERSION"
```
