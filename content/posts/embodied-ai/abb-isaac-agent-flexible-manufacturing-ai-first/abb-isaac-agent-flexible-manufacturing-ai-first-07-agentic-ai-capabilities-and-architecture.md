...
落地上，这一层通常是事务型数据库 + 日志存储 + 对象存储的组合。状态快照、工艺配方和产品配置先结构化落库，历史案例和日志回放再逐步补强检索能力。

主要包括：

* 状态快照：记录系统此刻的结构化状态，例如当前工步、当前阶段、异常码、视觉结果、机器人模式和重试次数，解决“现在发生了什么”。
* 历史案例：沉淀带结果的相似案例，帮助系统判断以前遇到类似情况时通常怎么处理、哪条恢复路径成功率更高。
* 日志回放：保留执行过程中的关键事件、输入、决策和回执，方便事后还原现场，判断问题到底出在感知、判断还是执行。
* 工艺配方：保存不同工步和料号对应的参数边界与工艺条件，例如容差、速度、力度、等待条件和允许动作范围。
* 产品配置：描述当前产品版本、零件组合、启用功能和现场设备差异，保证系统知道“这次任务面对的到底是哪一种产品和部署环境”。
...
* 哪条恢复路径曾经成功：比较不同恢复动作在历史上的结果，看看重拍、重试、回安全位或转人工哪种更可靠。
* 哪类料号或工艺容易在什么阶段失败：沉淀出“某些产品、某些工艺、某些阶段更容易出问题”的模式，帮助系统提前提高警惕。
...
* 某个料号允许的偏差范围：说明某类零件或产品能接受多大的位置、姿态或尺寸偏差，超出范围就不能继续执行。
* 某个工步的速度、力度、等待条件：规定每一步允许用多快的速度、多大的力、需要等多久或满足什么条件后才能进入下一步。
* 某类恢复动作是否允许在本工位执行：明确哪些补救动作可以自动做，哪些动作在这个工位根本不允许，必须停机或转人工。

### 4.5 产品配置

产品配置决定的是产品和任务模板的差异，例如：

* 产品版本：说明当前生产的是哪一版产品，不同版本往往对应不同结构、参数和工艺要求。
* 零件组合关系：描述哪些零件必须一起出现、先后怎么装、彼此之间有什么匹配关系，避免系统把不兼容的零件组合到一起。
* 工位是否启用某类功能：说明当前工位有没有打开某些能力，例如是否启用视觉复检、自动补偿或某类恢复动作。
* 当前部署现场有哪些可用设备：记录现场到底接了哪些机器人、相机、治具或外围设备，系统才能知道当前能调用哪些能力、不能依赖哪些能力。
