...
摘要：这一章顺着功能架构图往下拆。只有把每个框各自负责什么讲清楚，`Agentic AI` 该放在哪里。
...
* `Agentic AI` 不是一个单点模型
* 它也不直接接管机械臂控制
* 它是一套把任务理解、状态判断、技能编排和工业执行串起来的系统
...
领域扩展层不做通用推理，主要任务是把行业规则、工艺边界和专用技能钉死。
...
接着看智能体核心。任务理解、阶段判断和后续决策都在这里形成，但执行权不在这里。

首版实现更适合用 `大语言模型 + 领域技能`。`LLM` 负责理解任务、整理上下文、生成候选决策，`领域技能` 负责把可执行能力、工艺边界和恢复路径收成受限集合，别让智能体直接滑向自由动作生成。

图中央第二层是 `智能体核心`，真正起作用的是：
