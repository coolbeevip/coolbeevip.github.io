...
* 架构图源文件：[abb-isaac-agent-flexible-manufacturing-ai-first-07-agentic-ai-capabilities-and-architecture.puml](./diagrams/abb-isaac-agent-flexible-manufacturing-ai-first-07-agentic-ai-capabilities-and-architecture.puml)
...
后文统一使用一个固定任务，把入口、判断、编排、执行和回流串成同一条链：
...
本节定义任务入口层。所有上层请求都先在这里被整理成结构化任务，再进入后续判断和编排链路。

图的最上层是 `HMI / 工单`，里面有四个入口：
...
本节定义领域扩展层。它不负责通用推理，而是负责把行业规则、工艺边界和专用技能固定下来。
...
本节定义智能体核心。任务理解、阶段判断和后续决策都在这里形成，但底层执行权不在这里。
...
本节定义状态与知识底座。它负责把当前状态、历史经验和工艺配置持续送回决策链。
...
本节定义技能编排层。它把上层建议收成受限技能、明确参数和可执行流程。

图中央第三层是 `技能智能编排`，包含四个框：
...
本节定义感知与推理层。它负责把视觉、空间关系和状态演化结果转换成决策可用的事实。
...
本节定义技能运行时。它的职责不是再做一次高层判断，而是把编排结果稳定映射到底层接口。
...
本节定义执行边界。控制器负责运行已验证控制逻辑，现场设备负责把动作真正兑现出来。
...
本节补上执行回流主线。系统不是一次性下发动作，而是根据回执不断更新状态并重做决策。
...
本节把前面的模块重新收束成一套完整系统，说明每一层为什么存在，以及它们如何首尾相接。
