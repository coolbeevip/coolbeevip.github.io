...
摘要：这一章直接沿着功能架构图往下讲。图里的每个框都对应一类职责，拆开看，才知道 `Agentic AI` 应该放在哪，哪些事情又必须留在控制器和现场设备一侧。
...
这张图描述的是一套具身智能系统的分层架构，不是简单的调用顺序。
...
* `Agentic AI` 不是一个单点模型
* `Agentic AI` 也不是直接控制机械臂的总控大脑
* 它是一套由任务理解、状态判断、技能编排、运行时映射和工业执行共同组成的系统
...
这些边界一旦混掉，系统通常会往下面这些方向失控：

* 把大模型当控制器
* 把智能体当作只有聊天能力的助手
* 把行业规则、技能逻辑和底层接口全都揉进一个模块里
...
下面按架构分层展开，顺序从任务入口开始，经过决策、编排和执行，最后回到状态回流。

## 1. HMI / 工单：任务从哪里进入系统

先看任务入口。所有上层请求都先在这里收成结构化任务，再进入后面的判断和编排链路。

落地时，这里通常就是 `Web HMI + API 服务 + 消息队列` 的组合。核心不是界面复杂，而是尽快把工单、调度和人工输入收成统一任务对象。

图的最上层是 `HMI / 工单`，上层输入会从这里进入：
...
这里定义的是任务入口，不是前端展示。输入来源、任务上下文和约束条件都要在这一层落成结构化对象。
...
任务如果在这里没有结构化，后面的判断和编排都缺基础。

放到 `FPC` 插入这个例子里，这里收到的不是一句“帮我插一下排线”，而是一份结构化请求，例如：
...
再看领域扩展层。它不做通用推理，主要负责把行业规则、工艺边界和专用技能钉死。

落地时，`领域定义` 适合用 `YAML/JSON` 配置，结合版本管理和数据库维护；`领域技能` 更像技能注册表，通常也是配置加数据库，用来管理技能参数、前置条件和恢复路径。
...
这一块决定核心架构的可复用性。行业知识如果直接写进核心，系统很快会被某个工位、某类产品或某套设备绑定。
...
接着是智能体核心。任务理解、阶段判断和后续决策都在这里形成，但底层执行权不在这里。

首版实现更适合采用 `大语言模型 + 领域技能` 的组合。`LLM` 负责任务理解、上下文整理和候选决策生成，`领域技能` 负责把可执行能力、工艺边界和恢复路径收成受限集合，避免智能体直接滑向自由动作生成。

图中央第二层是 `智能体核心`，这里真正起作用的是：
...
这里是 `Agentic AI` 的核心决策层。

### 3.1 任务理解

`任务理解` 负责把入口层送来的请求翻译成系统内部可操作的目标。
...
这里的“长程”指任务级规划，不是底层连续控制。

放到这个例子里，`长程规划` 关心的不是末端执行器如何走轨迹，而是当前应走哪条恢复链：
...
`状态判断` 在具身场景里经常被低估，但它直接决定后续路径是否成立。
...
`后续决策` 负责给出下一步建议，但这里的“决策”不是自由生成底层动作。
...
技能编排层开始真正收紧自由度。上层建议会在这里被收成受限技能、明确参数和可执行流程。

这层最适合用 `技能注册表 + 规则引擎 + 行为树/状态机` 落地。对机器人场景来说，`行为树` 往往比通用工作流更贴近执行编排，尤其适合条件分支、回退和恢复；首版重点不是“规划得多聪明”，而是技能边界、参数来源和恢复路径要稳定。

图中央第三层是 `技能智能编排`，收敛主要发生在这里：
...
这一层位于智能体核心和技能运行时之间，负责把建议收成受限技能和可执行流程。

### 5.0 技能到底长什么样

前面一直在提“技能”，这里先把技能对象写实一点。

在这张架构图里，技能不是一句自然语言命令，也不是一段临时生成的动作文本。一个可编排、可执行、可回执的技能，至少应包含下面这些字段：
...
这样一写，`技能智能编排` 和 `技能运行时` 的关系就清楚了：

* 编排层负责决定“选哪个技能、补哪些参数、走哪条恢复路径”
* 运行时负责决定“把这个技能对象如何映射到真实控制接口并拿回结果”

### 5.1 技能选择

`技能选择` 负责把上层给出的后续决策，映射成具体技能类别。
...
`行为规划` 更适合直接理解成行为树编排。走到这里，系统不再只是决定“用哪个技能”，而是要把技能、条件检查、恢复动作和 fallback 组织成一棵可以执行的树。
...
高层建议在这里被收成一棵可执行的行为树，而不是一串线性的动作列表。

这里还要把边界说清楚：`技能智能编排` 输出的不是“下一条命令”，而是一棵带节点状态、前置条件和恢复分支的行为树实例。到了运行时，真正被执行的是这棵树里的叶子节点；运行时根据节点返回的 `success / failure / running` 持续 tick 整棵树，再决定下一步该走主路径、重试分支还是 fallback。

### 5.4 约束检查

这里第一次出现 `约束检查`。

这个 `约束检查` 更偏决策前检查，它负责判断：
...
这一层还没进入底层运行时，但允许的动作空间已经被明确收紧。

放到这个例子里，这里会把抽象建议真正收成一个受限方案：
...
技能运行时不再做高层判断，它负责把编排结果稳定落到底层接口。

工程上，这里通常会做成独立运行时服务，输入是技能对象或动作对象，输出是结构化回执。调度、超时、重试和失败上抛都在这一层收口。

如果上一层输出的是行为树，那么这一层承担的就是行为树执行器角色：负责维护树状态、tick 当前节点、执行叶子技能、消费回执，再把节点状态回写到树上。编排层负责“长成什么树”，运行时负责“这棵树怎么跑”。

图中央第四层是 `技能运行时`，执行链在这里落到接口：
...
如果这里说不清，整套架构还是停留在概念层。真正决定系统能否落地的，不是“有没有 Adapter”，而是：
...
* `Robot Adapter` 也不直接接收高层任务描述
* `Robot Adapter` 接收的是结构化动作对象，或者说动作协议对象

这个对象的作用，是把上层技能编排结果压缩成一份面向执行接口的统一描述。它至少应包含：
...
从工程实现看，这一层往往也是整套系统里工作量最大的部分。原因不在于算法最复杂，而在于它同时卡在两边：上面接统一动作对象和行为树执行状态，下面接 `ABB`、`ROS2`、视觉系统和 `PLC` 的真实接口，中间还要处理参数翻译、协议差异、超时、互锁、异常码和回执标准化。很多项目真正拖慢进度的，不是模型，而是这一层没有先收敛。

更稳的结构通常不是把它写成一个“大 Adapter”，而是拆成几块：
...
否则，行为树执行、协议转换、设备异常和回执处理很容易全部缠在一个服务里，后面几乎没法维护。
...
`参数绑定` 负责把上层给出的参数，绑定到具体控制接口的字段格式。
...
放到这个例子里，真正落到系统接口时，会被翻译成这样的真实请求：
...
只有把这里定义清楚，`技能运行时 -> Robot Adapter -> 控制器` 才是一条可实现的软件接口链，而不只是示意图。

## 8. 控制器与现场设备：真正执行发生在哪里

到了执行边界，控制器负责运行已验证控制逻辑，现场设备负责把动作真正兑现出来。

落地原则很简单：控制器沿用现有工业控制栈，现场设备继续由自动化系统接管，上层不要越过这条边界。真正需要补的是标准接口和回执，不是重写控制器。
...
这里补上执行回流主线。系统不是一次性下发动作，而是根据回执不断更新状态并重做决策。

实现上，这条链通常依赖统一回执对象和事件流。执行结果先写回状态快照，再触发下一轮状态判断和后续决策，这样系统才是闭环。
...
因此，`历史案例` 和 `日志回放` 的价值不只是事后复盘，它们首先服务于在线闭环：

* 当前执行结果要先回到状态里
* 状态更新后系统才有资格决定下一步

所以这张架构图描述的，本质上是一个持续决策系统，不是一次性流水线。

## 10. 架构如何收束成一个可落地系统

最后把前面的模块重新收束成一套完整系统，说明每一层为什么存在，以及它们如何首尾相接。
...
* 上层负责理解、判断、编排和调度
* 下层负责控制、执行和回执
* 中间通过技能对象、参数、约束和适配器把两者稳定连接起来

这也是它适合柔性制造首版系统的原因：既能把 `Agentic AI` 引进来，又不要求系统一上来就越过控制器和工业安全边界。

## 11. 最值得先验证什么

首版实现不应该平均铺开，而应先验证最容易决定成败的几个接口和闭环：

1. `HMI / 工单` 的输入能不能稳定收成结构化任务包。
2. `状态快照`、`工艺配方`、`产品配置` 能不能组成稳定上下文。
3. `技能选择`、`参数补全`、`行为规划` 能不能收成固定接口。
4. `技能运行时` 到 `Robot Adapter` 的接口能不能稳定映射到现场控制栈。
5. 底层回执能不能回流成 `历史案例` 与 `日志回放`。

这五件事一旦打通，这张图就不只是架构图，而会开始变成真实系统。

## 12. 下一步

下一步不是继续扩概念，而是挑一个具体场景，用这张图走通一次最小闭环。比较合适的切入点通常是：
