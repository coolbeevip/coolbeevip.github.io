---
title: "ScienceWorld 长程任务评测：从“回答科学问题”到“改变世界状态”"
date: 2026-06-12T10:00:00+08:00
summary: "介绍 ScienceWorld 长程任务评测机制，并用 melt water 任务说明 agent 如何从回答科学问题转向改变世界状态。"
tags: [ai, agent, embodied-ai, planning, scienceworld]
categories: [embodied-ai]
draft: true
---

# ScienceWorld 长程任务评测：从“回答科学问题”到“改变世界状态”
...
## ScienceWorld 的成功机制是什么

ScienceWorld 的成功机制和 ALFWorld 不太一样。ALFWorld 更常见的判断是看 `won` 字段；ScienceWorld 的 Python API 每一步返回的是观察、reward、是否完成和 info。
...
while True:
    ...
    obs, reward, is_completed, info = env.step(action)
...
`score` 是当前任务分数。ScienceWorld 内部原始分数按 `0-1` 计，Python wrapper 会把它乘以 `100`，所以 agent 看到的通常是 `0-100`。`reward` 不是另一个独立裁判，而是本步分数增量，也就是当前 `score` 减去上一步 `score`。如果某个动作让任务进度推进，reward 可能为正；如果没有推进，reward 可能是 0；如果任务进入坏状态，也可能触发负分和终止。

`is_completed` 表示 episode 是否结束。它可能因为任务完成而结束，也可能因为超过步数上限而结束，或者因为分数变成负数而提前结束。所以评估时不能只看 `is_completed == True`。更稳的判断是同时看最终 `score`。对完整成功来说，通常要看到任务完成反馈，并且最终分数达到任务满分，也就是接近 `100`。

以 melt water 为例，环境不是让 agent 最后输出"我完成了融化"。agent 必须一步步改变世界状态：生成 water，focus 到目标 substance，冷冻成 ice，再加热融化。每个动作后环境更新内部状态，任务 scorer 检查目标条件和中间进度。最后如果状态满足目标，观察文本里会出现 `Task Completed`，info 里的 `score` 会反映最终完成度。

所以 ScienceWorld 的评测核心不是 `won`，而是：

```text
action -> environment state changes -> scorer updates score -> is_completed says episode ended
```

这也是为什么看一条轨迹时，不能只看最后一句有没有"完成"字样。要看最终分数、reward 变化、是否超步数、以及任务反馈是不是由环境 scorer 给出的。
