...
这篇文章只讨论一个问题：`ScienceWorld` 这类基准测试到底在评估什么，以及它为什么能暴露普通 `LLM agent` 和具身智能 agent 之间的差异。
...
官方仓库和论文中，`ScienceWorld` 包含 `30` 个任务，覆盖 `10` 类小学科学主题，包括物体属性、热学、电学、化学变化、生物生命周期、植物生长、遗传等。每个任务还有多个参数化变体，用来减少“背答案”的可能。
...
| 输入是一道题 | 输入是一个可交互环境 |
| 输出是答案或解释 | 输出是一串动作 |
| 知识正确通常就够了 | 必须让环境状态达到目标 |
| 错误主要是推理错 | 错误可能来自找错物体、漏前置条件、等待不足、过早提交 |
| 不关心动作后果 | 每一步动作都会改变后续状态 |

所以，`ScienceWorld` 虽然是文本环境，但它评估的不是纯文本能力。它把物体、房间、容器、设备、温度、时间、状态变化都做成环境状态。agent 必须通过动作改变这些状态。

一个普通 `LLM` 可以回答：

```text
冰在温度升高到 0 摄氏度以上时会融化成水。
```

但在 `ScienceWorld` 中，这个回答没有直接得分。agent 要找到冰，或者先制造出冰，再把它放到热源上，等待状态变化，最后通过观察确认任务完成。

## ScienceWorld 的一次评估由什么组成

从工程角度看，`ScienceWorld` 的一次测试可以拆成六个要素。

| 要素 | 含义 |
| --- | --- |
| 任务描述 | 自然语言目标，例如 `melt water` |
| 初始环境 | agent 所在房间、可见物体、门和容器状态 |
| 动作空间 | 文本动作，例如 `go to kitchen`、`open freezer`、`move metal pot to stove` |
| 环境反馈 | 每个动作后的文本观察，包括成功、失败、状态变化 |
| 隐含状态 | 物体位置、容器关系、设备开关、温度、物态等 |
| 完成条件 | 环境判断目标状态是否达成，并返回 `Task Completed` |

这里最关键的是“隐含状态”。agent 看到的是文本，但环境内部维护的是可执行世界状态。例如：
...
这里需要先把边界说清楚：`ScienceWorld` 不是工业机器人基准测试，也不是人形机器人测试场。它没有真实视觉、机械臂控制、抓取姿态、碰撞检测、力控、PLC、安全互锁、产线节拍和工艺参数。

所以，不能从 `ScienceWorld` 分数直接推出一个系统能做好工业机器人。

它的价值在另一个层面：它把具身智能的高层任务问题抽象出来，让我们能单独观察 agent 是否会管理世界状态、动作后果、阶段推进、观察校验和失败恢复。

制造业流水线机械臂和开放场所人形机器人都属于具身智能，但它们不是同一个问题。

```text
制造业更像是在强约束下可靠完成任务。
开放场所机器人更像是在开放世界里理解和行动。
```

`ScienceWorld` 处在更高抽象的一层。它不验证机器人本体能力，但可以帮助我们理解：一旦智能体从“回答问题”进入“改变世界”，系统就必须处理状态、动作、时间和反馈。后面用 `melt water` 讲的正是这部分能力。
...
这个任务容易误解。它不是让 agent 解释“冰怎么融化”，也不是让 agent 直接加热一杯水。任务叫 `melt water`，但液态水本身不能被 melt。一个可行策略是：
...
第一，环境里有大量无关物体。`potato`、`apple`、`soap`、`painting` 都会出现在观察里，但和任务无关。agent 不能只做关键词匹配。
...
| --- | --- |
| `thermometer` | 检查温度变化 |
| `metal pot` | 装水，承载目标 substance |
| `sink` | 产生 water |
| `freezer` | 降温，把 water 变成 ice |
| `stove` | 加热，把 ice 融化 |

其中 `metal pot` 一开始不在可见列表里，需要打开 `cupboard` 才能拿到。这就是前置条件。
...
* `thermometer` 用于后续状态验证。
* `metal pot` 用于承载 water / ice。
...
| 测试点 | 在 melt water 中的表现 |
| --- | --- |
| 目标解释 | `melt water` 实际要求先制造可融化对象 |
| 房间搜索 | 从 hallway 找到 kitchen |
| 对象发现 | 打开 cupboard 才能拿到 metal pot |
| 工具选择 | thermometer 用于确认温度，pot 用于承载 substance |
| 状态生成 | sink 动作生成 water |
| 任务绑定 | `focus on substance in metal pot` |
| 过程控制 | freezer + wait 让 water 变 ice |
| 动作后果 | stove + wait 让 ice 融化 |
| 校验 | examine 和 thermometer 确认状态变化 |
| 恢复 | 对无效动作和冗余动作继续推进 |
...
| 维度 | 普通 LLM 智能 | ScienceWorld / 具身智能 |
| --- | --- | --- |
| 目标 | 生成正确解释 | 改变环境状态 |
| 输入 | 文本问题和上下文 | 房间观察、物体状态、动作反馈 |
| 输出 | 答案、计划、工具参数 | 可执行动作 |
| 成功标准 | 回答是否合理 | 世界状态是否达成目标 |
| 记忆 | 对话历史 | 对象位置、容器关系、设备状态、阶段进度 |
| 错误处理 | 重新回答或修改计划 | 根据环境反馈恢复路径 |
...
```text
找到锅 -> 接水 -> 聚焦 water -> 放进 freezer -> 等到变 ice
-> 放到 stove -> 加热 -> 观察 Task Completed
```

所以，具身智能的关键不是“懂不懂概念”，而是能不能把概念变成一串可执行、可验证、可恢复的状态转换。
...
| 动作 | 前置条件 | 预期效果 | 失败或无效情况 |
| --- | --- | --- | --- |
| `open cupboard` | agent 在 kitchen，cupboard 可见 | cupboard 变 open，内部物体可见 | cupboard 已经打开时无状态变化 |
| `pick up metal pot` | metal pot 可见且可拿 | metal pot 进入 inventory | metal pot 不可见或在封闭容器里 |
| `activate sink` | metal pot 在 sink 中 | metal pot 中生成 water | 没有容器时可能无关或无效 |
| `move metal pot to freezer` | freezer 可访问，pot 可移动 | pot 进入 freezer | freezer 未打开或 pot 不在手里 |
| `activate stove` | pot 在 stove 上 | 开始加热 pot 中 substance | stove 上没有目标容器 |
...
| 记忆类型 | 内容 | melt water 示例 |
| --- | --- | --- |
| 短期观察 | 最近一次环境反馈 | freezer open，pot in freezer |
| 工作记忆 | 当前阶段需要的信息 | 正在等待 water 变 ice |
| 状态记忆 | 稳定世界状态 | pot contains water，focused on substance |
| 失败记忆 | 刚发生的无效动作 | 不能把 metal pot 倒进 metal pot |
| 任务记忆 | 目标和完成条件 | melt focused substance |
...
`melt water` 不能只靠一次性计划。它更适合做成阶段控制。

| 阶段 | 进入条件 | 退出条件 |
| --- | --- | --- |
| 找到厨房 | agent 在 hallway | agent 到达 kitchen |
| 准备工具 | kitchen 可观察 | thermometer 和 metal pot 可用 |
| 生成 water | metal pot 可用，sink 可用 | pot contains water |
| 聚焦目标 | pot contains water | focused_object 是 substance in metal pot |
| 冷冻 | pot contains water，freezer 可用 | substance 变成 ice |
| 加热 | substance 是 ice，stove 可用 | substance 融化 |
| 完成校验 | 观察到目标状态 | 环境返回 Task Completed |

阶段控制的价值在于防止 agent 做错时机的动作。

例如：

* 还没有 water 时，不应该去 freezer 等待。
* water 还没有变 ice 时，不应该启动 stove。
* ice 已经形成后，不应该继续无限冷冻。
* stove 未启动时，不应该期待融化。
...
具身任务不能只记录“动作已经执行”。它必须确认动作后果是否发生。

在 `melt water` 中，关键校验点包括：

| 校验点 | 观察方式 | 目的 |
| --- | --- | --- |
| metal pot 是否拿到 | inventory / look around | 确认容器可用 |
| pot 是否有 water | examine substance in metal pot | 确认 sink 动作有效 |
| 温度是否下降 | thermometer | 判断 freezer 是否生效 |
| water 是否变 ice | examine ice | 判断是否可以进入加热阶段 |
| stove 是否开始加热 | stove 状态 / 后续温度 | 判断 heat source 是否生效 |
| 任务是否完成 | 环境返回 Task Completed | 最终验收 |
...
`move metal pot to freezer` 被接受，只能说明锅进了 freezer。它不能说明 water 已经变成 ice。必须 `wait`，再观察。
...
| 问题 | 环境反馈 | 恢复策略 |
| --- | --- | --- |
| 门已经打开 | door already open | 不重复开门，直接 go |
| 物体不可见 | 找不到目标对象 | look around，打开容器，换房间搜索 |
| 动作无效 | 不能把物体放进自己 | 放弃该动作，回到状态目标 |
| 等待后状态未达成 | water 仍未变 ice | 继续等待或确认 freezer 条件 |
| 设备未生效 | 温度不变 | 检查容器位置、设备开关 |
| 最终未完成 | 没有 Task Completed | 重新校验 focused object 和目标状态 |
...
```text
稳定状态：pot contains water，pot in freezer，focused on substance
当前问题：wait 后仍是 water
下一步：继续 wait，或检查 thermometer，确认 freezer 是否仍有效
```
...
```text
LLM:
  负责语言理解、目标解释、候选计划、异常解释

Agent runtime:
  负责状态管理、动作约束、阶段控制、观察校验、恢复策略

后训练 / 策略训练:
  让模型从轨迹中学会在特定环境里稳定选择动作
```

如果只是 demo，`LLM + prompt + few-shot trajectory` 可能够用。

如果要稳定完成大量 `ScienceWorld` 变体，通常需要加入轨迹学习、行为克隆、强化学习、过程监督或反思数据微调。后训练的目标不是补科学知识，而是让模型学会在环境反馈中行动。
